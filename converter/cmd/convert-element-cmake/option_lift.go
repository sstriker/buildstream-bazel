...
func finishOptionLift(job *optionLiftJob, a cli.Args, hostBuildDir string, r *fileapi.Reply, pkg *ir.Package) {
	if job == nil {
		// Even a declined pre-gate (nothing liftable) must satisfy a
		// requested --out-option-settings: the write-a pipeline
		// declares the file as a genrule output.
		if len(a.LiftOptions) > 0 {
			writeOptionSettings(a.OutOptionSettings, nil, nil)
		}
		return
	}
	defer job.cleanup()
...
// when the flag is unset). When NOTHING lifted, a header-only
// placeholder is written instead of skipping: the write-a pipeline
// declares options-BUILD.bazel as a genrule output, and Bazel
// requires declared outs to exist (the rejections.json precedent).
// stage-b recognizes the placeholder (no flag rules) and stages
// nothing. Failures warn rather than failing the convert — the BUILD
...
func writeOptionSettings(outPath string, lifted []optionsettings.Option, groups []optionsettings.Group) {
	...
	if body == nil {
		body = []byte("# Generated by convert-element-cmake. DO NOT EDIT.\n#\n# No options lifted (--lift-options matched nothing liftable).\n")
	}
