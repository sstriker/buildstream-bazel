...
func recoverConfigureFilesFromCalls(calls []shadow.ConfigureFileCall, hostSrcDir, recordedSrcDir, hostBuildDir, recordedBuildDir string, dirScopes []dirScope, liftEnabled bool, cmakeVars map[string]string, cc *codegenContext) ([]configureFileOut, error) {
	...
	for _, call := range calls {
		...
		if cc.outputClaimed(rel) {
			// Another lifter already claimed this output path — most often
			// execute_process's regenerating producer (it runs before
			// configure_file recovery), or file(GENERATE) / a recovered genrule.
			// Two producers emitting the same rel land duplicate rule names +
			// colliding outs (Bazel rejects "output generated by both"), and the
			// consumer would wire to whichever registered OutToGenrule last —
			// preferring THIS frozen configure_file bake over a regenerating
			// producer. Defer: skip the emit and let the existing producer own the
			// output (mirrors recoverFileGenerate's outputClaimed guard).
			continue
		}
