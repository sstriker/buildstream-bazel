...
// TestToIR_ExecuteProcessTouch_Lifts is the end-to-end regression for
// the raw-`touch` lift (the POSIX analog of `cmake -E touch`): a target
// plus a configure-time `touch ${BINARY}/marker` execute_process call
// must lower cleanly (no unsupported-execute-process Tier-1) and land
// an empty-file genrule in the package. Mirrors the raw-cp e2e test.
func TestToIR_ExecuteProcessTouch_Lifts(t *testing.T) {
	hostSrc := t.TempDir()
	if err := os.MkdirAll(filepath.Join(hostSrc, "src"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	traceRaw := []byte(
		`{"args":["COMMAND","touch","` + build + `/marker.stamp"],"cmd":"execute_process","file":"` + filepath.Join(hostSrc, "CMakeLists.txt") + `","line":7}` + "\n",
	)
	pkg, err := lower.ToIR(r, nil, lower.Options{
		HostSourceRoot: hostSrc,
		BuildDir:       build,
	...
	if gen == nil {
		t.Fatalf("expected a touch genrule in the package; targets: %+v", pkg.Targets)
	}
	if len(gen.GenruleOuts) != 1 || gen.GenruleOuts[0] != "marker.stamp" {
		t.Errorf("genrule outs: %v want [marker.stamp]", gen.GenruleOuts)
	}
	if !strings.Contains(gen.GenruleCmd, "touch") {
		t.Errorf("genrule cmd should touch the file; got %q", gen.GenruleCmd)
	}
}
