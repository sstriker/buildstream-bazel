...
// TestRecoverFileGenerate_RelativeOutputAnchored: a relative OUTPUT resolves
// against the registering scope's CMAKE_CURRENT_BINARY_DIR (cmake docs), so
// the recovery anchors it via the codemodel directory scopes — previously
// such calls were dropped outright ("no per-call binary-dir context"),
// silently losing a generated file. A sub-scope call anchors at the sub's
// build mirror; without dirScopes (offline) the historical drop remains.
func TestRecoverFileGenerate_RelativeOutputAnchored(t *testing.T) {
	template := "#define BANNER \"hi\"\n"
	rendered := []byte("#define BANNER \"hi\"\n")
	hostSrc, hostBuild := fileGenerateTestSetup(t, "sub/banner.h.in", template, "sub/banner.h", rendered)
	calls := []shadow.FileGenerateCall{{
		File:     filepath.Join(hostSrc, "sub", "CMakeLists.txt"),
		Output:   "banner.h", // relative → sub scope's binary dir
		Input:    filepath.Join(hostSrc, "sub/banner.h.in"),
		HasInput: true,
	}}
	...
	if len(out) != 1 || out[0].RelOutput != "sub/banner.h" {
		t.Fatalf("outs = %+v, want one entry at sub/banner.h (the sub scope's binary dir)", out)
	}
	...
	if len(out2) != 0 {
		t.Fatalf("without dirScopes the relative output should drop (offline fallback); got %+v", out2)
	}
}
