...
// writeToolchainConfigConstants emits the static action-set constants and the
// two feature-builder helpers (_feature_with_flags, _default_compile_flags_feature)
// shared by every generated toolchain config. These are byte-identical across all
// models — only the feature *list* assembled in _impl (below) is model-derived.
func writeToolchainConfigConstants(buf *bytes.Buffer) {
