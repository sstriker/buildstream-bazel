...
func Parse(argv []string, stderr io.Writer) (Args, int) {
	...
	fs.BoolVar(&a.TwoPassGenex, "two-pass-genex", true, "when the first configure pass leaves arbitrary genex literals the structural probe + Go-side evaluator can't resolve, run a warm second cmake configure against the same build dir that resolves them via file(GENERATE), then re-lift. Conditional (skipped when nothing is unresolved — zero overhead in the common case) and warm (reuses the first pass's try_compile/find_package cache). Source-root mode only; requires cmake 3.24+. Pass --two-pass-genex=false to disable.")
