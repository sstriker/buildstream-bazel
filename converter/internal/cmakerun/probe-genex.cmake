...
function(_cmtb_probe_genex)
    ...
    # De-duplicate the collected target list before emitting any
    # file(GENERATE). The same target name can be collected twice — a
    # source directory add_subdirectory()'d into two binary dirs (a
    # vendoring pattern) is listed twice under the parent's
    # SUBDIRECTORIES, so its BUILDSYSTEM_TARGETS are appended once per
    # listing. A repeated name would emit two file(GENERATE) calls with
    # the SAME OUTPUT path (cmake-to-bazel.genex/<tgt>/…), which cmake 4
    # rejects as a hard error ("Files to be generated by multiple
    # different commands") — cmake 3.x tolerated the identical-content
    # duplicate, cmake 4 does not. Deduping keeps one probe per name; a
    # name that resolved to distinct targets across the two adds keeps
    # the first, and the others degrade to the legacy genex fallback
    # (ReadGenexProbe tolerates a missing per-target dir) rather than
    # aborting the whole configure.
    list(REMOVE_DUPLICATES _CMTB_TGTS)
