...
| `cmake-codegen-execute-process-op=<op>` | execute_process-derived cmake -E call carries the op name so audits can split lifted ops without re-parsing the cmd. cmake -E builtins use the op name (`touch` / `copy` / `copy_if_different` / `create_symlink` / `configure_file`); raw POSIX commands lifted via their cmake -E equivalent use the raw driver basename (`cp` → copy, `ln` → create_symlink-as-copy; raw `touch` shares the `touch` label since it is the same operation). | append-only |
