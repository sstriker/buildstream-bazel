...
| `cmake-codegen-genex-target-objects-refused` | file(GENERATE) whose body references `$<TARGET_OBJECTS:t>` where `t` is **not a same-element `OBJECT_LIBRARY`** (cross-element / imported / wrong type — Bazel exposes a target's `.o` files only for an object library this element builds, via a `compilation_outputs` filegroup), or where the (a)-evaluator couldn't resolve the template. The emitted genrule is a **refusal stub** that fails at bazel-build time rather than baking cmake's recording-machine object paths (which don't exist on the executor). The resolvable case (same-element OBJECT_LIBRARY) instead emits a `filegroup(output_group = "compilation_outputs")` sibling and resolves normally (`-resolved`). Not a resolution — kept distinct. | append-only |
