...
| **curl** | `ok` (library + CLI build green, 234 actions, no cmake) | Converts **clean — 0 rejections**; **`bazel build //...` is GREEN** — libcurl + the full curl CLI tool. Two generic converter fixes drove it: **(1) cross-package generated-output visibility** — curl's `tool_hugehelp.c` genrule in `//src` consumes the `docs/cmdline-opts/curl.txt` output of a genrule in the root package; a genrule's outputs inherit its visibility, and the producer was private (`target … is not visible`). `emit/split` now publicizes any producer whose generated output is referenced directly (in srcs) by a consumer landing in a different package (`splitPlan.publicize`, keyed off `genOutProducer`; the generated-`.inc`-via-wrapper path is unaffected since that's consumed same-package). **(2) `add_definitions()` → `local_defines`** — cmake's directory-scoped `add_definitions(-DBUILDING_LIBCURL)` (lib/CMakeLists.txt) is PRIVATE (never exported via INTERFACE_COMPILE_DEFINITIONS), but the codemodel folds it into every in-directory target's effective `Defines` untagged, so the converter emitted it as a transitive Bazel `defines` that **leaked onto the curl tool** linking libcurl. The tool compiles the SAME sources WITHOUT `BUILDING_LIBCURL` so `lib/curl_base64.h` aliases `Curl_*`→`curlx_*` (avoiding symbol collisions with the linked libcurl); inheriting the macro left `var.c`'s `curlx_base64_encode` undeclared. The shadow trace now captures `add_definitions` and `lower` routes those defines to `local_defines` (a PUBLIC/INTERFACE `target_compile_definitions` of the same string still wins; the codemodel already scopes the macro to the right targets, so name-matching moves it precisely). `find_package(ZLIB)` lifts to the host `-lz`. The lens scopes to the **library + CLI** (`scripts/build-lens/curl.conf`): `BUILD_TESTING=OFF` (curl's `tests/` are `add_custom_target`s running the perl `runtests.pl` harness — phony test-RUN targets, not file producers) and `BUILD_LIBCURL_DOCS=OFF`+`ENABLE_CURL_MANUAL=OFF` (the `docs/` tree runs perl helpers over ~300 `.md` files — manpage generation, not the library). **Test/doc side** is a tracked follow-up: the generic **genrule-script-staging** fix (stage a genrule's invoked perl/python/sh script + its read inputs into `srcs`) would let the docs AND the perl test harness build (see ROADMAP). |
