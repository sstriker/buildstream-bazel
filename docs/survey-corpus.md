...
| **abseil** | compiles deep | `alias` `actual` relabeled cross-package under split + `;`-joined `target_link_libraries` deps split (#424). **PRIVATE-root-include fix:** a `target_include_directories(PRIVATE ${PROJECT_SOURCE_DIR})` resolved to `rel==""` but the converter handled the private/system split *before* the `rel==""` check, so it emitted a bogus bare `-I` and left `RootInclude` false — losing the `include_prefix` that resolves the target's own `absl/…` includes (e.g. `spinlock_wait`). Now `rel==""` is handled first (#431). **Remaining is BROAD, not cctz-specific** — `bazel build //... --keep_going` shows **~363 compile failures across ~10 packages** (strings/log/time/base/synchronization/…), all the same shape: an `absl/…`-rooted `#include` not resolving under split. Root cause: a `target_include_directories(${PROJECT_SOURCE_DIR})` (`rel==""`) triggers the root-walk hdr discovery, which pulls **all ~397 element headers into every such target's `hdrs`** (`spinlock_wait` and `time_zone` have byte-identical 397-hdr IR); under split the cross-package ones (343–378 per target) become `//…` labels → fail `allPackageLocalHdrs` → block `include_prefix` → the target's own `absl/…` includes don't resolve. abseil's targets have **no link deps** for these header-only cross-package includes, so the proper fix is **header-dependency inference** (a header→owning-target index → inject deps + reduce each target's `hdrs` to package-local → `include_prefix` applies). Plus standalone external test deps (`GTest::gmock`). In `ROADMAP.md`. |
