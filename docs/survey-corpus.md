...
| **llvm** | `skip(convert)` (in progress) | First whole-tree build-lens drive. Survey `$(LLVM_DIR)/llvm` (the subproject). 390 diagnostics rejections = 388 benign `unsupported-source-path` notices + 2 real `unsupported-execute-process`; `BUILD_LENS_IGNORE_REJ=1` bypasses the cost gate. The **strict** clean convert then aborts on **4 execute_process calls** in `cmake/modules/AddLLVM.cmake` that must each be handled before the build can run: (a) `cc -Wl,--version -o /dev/null` (linker-capability probe, :223) — classify as a droppable probe; (b) `cmake -E copy_if_different .../Extension.def.tmp → Extension.def` (:1286) and (c) `.../ExtensionDependencies.inc.tmp → .inc` (:1337) — **config.h-class**: a build-dir `.tmp`→output copy is semantically `configure_file(COPYONLY)`; the cmake-E copy handler (`execute_process.go`) refuses only because the src is under the build dir, so teach it to bake the on-disk output bytes like `classifyFileRename`; (d) `git rev-parse --git-dir` (VC stamp, :2588) — classify as a droppable/stamp probe (or `LLVM_APPEND_VC_REV=OFF`). Once strict-convert-clean, `bazel build //...` over the X86-only library graph is the next frontier (the leaf libs + tablegen tier already build per ROADMAP). `scripts/build-lens/llvm.conf`. |
