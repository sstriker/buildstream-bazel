...
| **llvm** | `FAIL` (build phase; in progress) | First whole-tree build-lens drive (survey `$(LLVM_DIR)/llvm`, the subproject). **Strict convert now clean** — the 4 `AddLLVM.cmake` `execute_process` blockers were each fixed *in the converter* (preferred over conf flags): (a) the `cc -Wl,--version` linker probe was mis-parsed (argv `/usr/bin/cc;-Wl,--version;-o;/dev/null` arrived as one `;`-joined token → driver `null`); `splitCMakeListArg` re-splits cmake list-valued COMMAND args so argv[0] is `cc` → probe → skip; (b)/(c) `cmake -E copy_if_different .../Extension.def.tmp → Extension.def` (and `ExtensionDependencies.inc`) bake the build-dir output bytes (`bakeBuildDirCopyOutput`, config.h-class — the cmake-E copy lifter no longer refuses a build-dir intermediate src); (d) `git rev-parse --git-dir` is a repo-*location* probe, not a value stamp — `gitRepoLocationQuery` classifies it `BucketProbe` → skip. With those, `bazel build //...` runs (rejections 390→194). **Remaining build-phase blockers:** (1) the **`.exports` genrule src umbrella/split desync** — root-caused: the build lens overlays cmakeSrc (`llvm/`) at `elements/llvm/`, and split places packages cmakeSrc-relative (single `llvm`, `elements/llvm/tools/remarks-shlib`) and re-relativizes cc_library srcs to package-relative — but `normalizeInput` gives genrule SOURCE-tree srcs the umbrella prefix (`llvm/tools/...`) while build-dir paths (the genrule's own output/package) stay single, so split relabels the src to a cross-package `//elements/llvm/llvm/tools/remarks-shlib:Remarks.exports` pointing at a package with no BUILD and (under the cmakeSrc overlay) a file that actually sits at the single-`llvm` path → "missing input file" (`Remarks.exports`, `bugpoint.exports`; the ROADMAP "source-tree-input==build-tree-output aliasing" item). Fix direction: strip the umbrella prefix when split relabels a genrule's source-tree src, consistent with its cmakeSrc-relative package placement + cc_library src re-relativization (intricate — shares machinery with zstd's umbrella path + the split-gate fixtures, so needs careful regression coverage). (2) `VCSRevision.h` VC-stamp header — parked via `LLVM_APPEND_VC_REV=OFF` (build-time stamp the lens doesn't need; faithful path is `cmake_configure_file` `stamp_values`). `scripts/build-lens/llvm.conf`. |
