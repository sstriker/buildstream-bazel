...
| **bde** | Bloomberg's BDE libraries — **metadata-driven** target construction: each package GROUP is built by one `bbs_setup_target_uor(${target})` that reads `*.mem` (component membership) + `*.dep` (group dependency) metadata via the `BdeBuildSystem` (BBS), instead of explicit `add_library`/`target_sources`. New stresses: **scale from a tiny CMakeLists** (`groups/bsl` alone → 53 `cc_library` + **729 per-component `.t.cpp` test-driver `cc_binary`s**, all enumerated from metadata the File-API codemodel already resolves — so the metadata novelty is invisible to the converter; the genuine stress is the target COUNT); a **`python3` test-runner `add_test`** shape (`add_test(COMMAND python3 <bbs runner> <driver>)`, so the 729 drivers don't lower to `cc_test` — a dropped-test-trees datapoint, ROADMAP); the **`bde_xt_cpp_splitter.py` / `sim_cpp11_features.pl` configure-time codegen** (the 1 `unsupported-execute-process` rejection — a side-effect generator with no captured output); and `bbs_emit_pkg_config` → generated `.pc`. Scoped to ONE group (`groups/bsl`) — surveyed at the BDE root with the other groups + `standalones/` + bsl-irrelevant thirdparty (inteldfp/pcre2) pruned via `CONFIGURE_PRUNE_SUBDIRS`. Structural lenses: rejections 1, idiom 0, coverage 7 (the `bsl+bslhdrs` umbrella interface lib's sibling-package edges — abseil-INTERFACE-deps class, ROADMAP), todos 2. Build lens self-skips (`skip(rej)`) until the splitter codegen recovers. `find_package(BdeBuildSystem)` is provided by the **bde-tools** checkout (same tag) via `CMAKE_PREFIX_PATH`; `make fetch-bde` clones BOTH. | github.com/bloomberg/bde (`BDE_VERSION`) + github.com/bloomberg/bde-tools (`BDE_TOOLS_VERSION`) | `make fetch-bde` |
