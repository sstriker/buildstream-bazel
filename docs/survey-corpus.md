...
| **curl** | `FAIL` (converts clean; library/CLI compiles 151/177; blocked on a curl-internal `curlx_*` header issue) | Converts **clean — 0 rejections**. One generic converter fix landed driving it: **cross-package generated-output visibility** — curl's `tool_hugehelp.c` genrule in `//src` consumes the `docs/cmdline-opts/curl.txt` output of a genrule in the root package; a genrule's outputs inherit its visibility, and the producer was private (`target … is not visible`). `emit/split` now publicizes any producer whose generated output is referenced directly (in srcs) by a consumer landing in a different package (`splitPlan.publicize`, keyed off `genOutProducer`; the generated-`.inc`-via-wrapper path is unaffected since that's consumed same-package). The lens scopes to the **library + CLI** (`scripts/build-lens/curl.conf`): `BUILD_TESTING=OFF` (curl's `tests/` are `add_custom_target`s running the perl `runtests.pl` harness — phony test-RUN targets, not file producers) and `BUILD_LIBCURL_DOCS=OFF`+`ENABLE_CURL_MANUAL=OFF` (the `docs/` tree runs perl helpers `cd2nroff`/`managen`/`mkhelp.pl` over ~300 `.md` files — manpage generation, not the library). `find_package(ZLIB)` lifts to the host `-lz`. **Remaining: (1)** the curl CLI's `src/var.c` calls `Curl_base64_encode`, which `lib/curl_base64.h` `#define`s to `curlx_base64_encode` (curl's mid-refactor `Curl_*`→`curlx_*` namespace), but the `curlx_*` prototype isn't visible to the tool build — a curl-version-specific header/define issue to diagnose. **(2)** a tracked generic converter follow-up: **stage a genrule's invoked-script inputs** (a cmd that runs `perl scripts/foo` needs `scripts/foo` in srcs) — would let curl's docs AND its perl test harness build (the "do the test side" coverage). |
