...
| **OpenBLAS** | `ok` | **Green on the C-LAPACK shape** — the whole ~2460-target kernel + LAPACK + BLAS library `bazel build //...`s with no cmake. Four converter features + one build-lens flag, each peeling the next blocker: (1) `stageGeneratedSourceRootIncludes` rewrites the ~1951 GenerateNamedObjects wrapper `#include "<srcroot>/…kernel.c"` to workspace-relative + stages the kernel (.c/.S) as a `textual_hdr`; (2) `textualIncludeClosure` stages the transitive micro-kernel chain (`caxpy.c`→`caxpy_microk_haswell-2.c`); (3) `classifyFileRename` recovers `config.h` (written via `file(RENAME)` in the cross-compile branch, not `configure_file`) as a COPYONLY bake folded into `root_headers`; (4) the compile-group split (`splitCompileGroups`) now partitions generated sources back into per-compile-group sub-cc_libraries by the codemodel's `SourceIndexes` — so the per-source `COMPILE_OPTIONS "-mfma"` FMA kernels (*rot_k*, dgemv_t_k) compile in their own sub-library WITH `-mfma` and the 65 generated `.S` ASM TUs compile WITHOUT the C-only `-mfma`/`BUILD_COMPLEX*` flags, matching cmake exactly (this retired the former `CMAKE_C_FLAGS=-mfma` build-lens flag, which had masked the gap build-wide); (5 flag) `NO_CBLAS=1` drops the CBLAS reference-test suite whose `cblas_xerbla` collides with the library under `alwayslink`. Compile-db fidelity: `copt_mismatch`/`define_mismatch` both 0 (each was 65 before #4). Follow-ups (faithful converter fix for #5; real reference-Fortran LAPACK) tracked above/below. `scripts/build-lens/openblas.conf`. |
