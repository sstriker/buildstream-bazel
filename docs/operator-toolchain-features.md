...
### Option 3 — declare a cc_toolchain feature

The Bazel-idiomatic answer for cross-target compiler flag bundles
is a cc_toolchain feature:

```python
# //toolchain/features.bzl
...
`cc_library` / `cc_binary` in projects using this toolchain
picks up the flags automatically. You can then file an issue
asking the converter to extend `toolchainfeature.Feature` to
recognise the project's specific flag set and emit
`features = ["project_warnings"]` instead of the raw copts.

**Trade-off**: the cc_toolchain definition is project-scoped, so
it doesn't compose cleanly across multiple converted projects in
the same Bazel workspace. Best fit for monorepo-style consumer
setups where one cc_toolchain owns the build conventions for
every contained project.
...
## Source layout — project B answers it

The converter emits a single BUILD.bazel for one cmake source
root. The operator's *project B* layout decides where that
BUILD lands in the Bazel workspace and what prefix the rendered
`srcs` paths carry. This is a per-project decision: pick the
workspace package that owns the cmake source tree, place the
BUILD.bazel there, and the converter's package-relative paths
(`include/foo.h`, `lib/Analysis/Foo.cpp`) align.

For the consumer-mode fidelity gate
([`scripts/run-fidelity.sh`](../scripts/run-fidelity.sh)), the
project B harness sets the workspace layout explicitly and
stages the consumer's `#include`s relative to that root. See
[`docs/fidelity-deltas.md`](fidelity-deltas.md) for the layout
contract.
