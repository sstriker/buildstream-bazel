...
| You changed | Run |
|---|---|
| Anything `*.go` | `go build ./...` then `go vet ./...` then `gofmt -l .` (must be empty) then `make staticcheck` then `make lint-complexity` then `go test ./...` (all five are blocking CI gates) |
