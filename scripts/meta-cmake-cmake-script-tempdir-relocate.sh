#!/bin/sh
# meta-cmake-cmake-script-tempdir-relocate.sh — render+build gate for the
# temp-dir-then-copy codegen recovery (cmake -E copy / copy_if_different form).
#
...
fixture="$repo_root/converter/testdata/sample-projects/cmake-script-tempdir-relocate"
work_dir="$(mktemp -d)"
...
echo "ok  meta-cmake-cmake-script-tempdir-relocate: temp-dir tool recovered to a regenerating genrule + lifted copy (no frozen copy-bake)"
...
if command -v bazel >/dev/null; then
    ...
    echo "ok  meta-cmake-cmake-script-tempdir-relocate: bazel not on PATH, skipping build half"
    exit 0
fi
...
if [ "$bazel_major" -lt 7 ]; then
    echo "ok  meta-cmake-cmake-script-tempdir-relocate: bazel < 7, skipping build half"
    exit 0
fi
...
module(name = "tdrelocate", version = "0.0.0")
...
echo "ok  meta-cmake-cmake-script-tempdir-relocate: //:app builds + runs from the recovered temp-dir tool genrule"
