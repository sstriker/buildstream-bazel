#!/bin/sh
# meta-cmake-protoc-cmake-script-wrapper-nested.sh — render+build gate for P3 of
# the wrapper-codegen coverage: the FULL recursion. protoc is hidden behind TWO
# nested `cmake -P` wrappers — the ninja edge runs `cmake -P gen_proto.cmake`,
# that script's execute_process is another `cmake -P inner_gen.cmake`, and the
# real protoc only appears one level deeper, inside inner_gen.cmake.
#
# Unlike P2 (a single one-level re-trace, meta-cmake-protoc-cmake-script-
# wrapper.sh), recovering this needs the recursion driver (expandCommandSources):
# re-trace gen_proto, recognize the nested `cmake -P inner_gen` as a wrapper,
# recurse into it forwarding the -D cache args, and only there bottom out at
# protoc. The nested execute_process runs protoc for real during the trace, so
# foo.pb.{cc,h} land in the absolute build dir — the on-disk evidence the
...
# P3 (--recognize-codegen --cmake-script-trace): the doubly-nested protoc lowers
# to proto_library + cc_proto_library; rewriteNativeRuleConsumers strips
# foo.pb.cc from use_foo and wires a //:foo_cc_proto deps edge. Then bazel-builds
# //:use_foo.
set -eu
...
fixture="$repo_root/converter/testdata/sample-projects/protoc-cmake-script-wrapper-nested"
work_dir="$(mktemp -d)"
...
echo "ok  meta-cmake-protoc-cmake-script-wrapper-nested: control refuses the nested cmake -P-wrapped protoc (unrecoverable without opt-in)"
...
echo "ok  meta-cmake-protoc-cmake-script-wrapper-nested: the recursion is gated on --cmake-script-trace (recognize-codegen alone still refuses)"

# --- P3: recurse through both wrappers, recognize the protoc, wire the consumer. ---
...
"$bin_dir/convert-element-cmake" --source-root "$fixture" \
    ...
    >"$rec/out" 2>"$rec/err" || fail "P3 convert (--recognize-codegen --cmake-script-trace) failed" "$rec/err"
b="$rec/BUILD.bazel"
...
echo "ok  meta-cmake-protoc-cmake-script-wrapper-nested: protoc behind two nested cmake -P scripts recovered -> proto_library + cc_proto_library + direct deps"

# --- Bazel-build half ---
if command -v bazel >/dev/null 2>&1; then BZL=bazel
elif command -v bazelisk >/dev/null 2>&1; then BZL=bazelisk
else echo "ok  meta-cmake-protoc-cmake-script-wrapper-nested: bazel not on PATH, skipping build half"; exit 0; fi
bzlmajor=$("$BZL" --version 2>&1 | head -1 | awk '{print $2}' | cut -d. -f1)
case "$bzlmajor" in [0-9]*) ;; *) bzlmajor=0 ;; esac
if [ "$bzlmajor" -lt 7 ]; then echo "ok  meta-cmake-protoc-cmake-script-wrapper-nested: bazel < 7, skipping build half"; exit 0; fi

ws="$work_dir/ws"; mkdir -p "$ws"
cp "$fixture/foo.proto" "$fixture/use_foo.cc" "$ws/"
cp "$b" "$ws/BUILD.bazel"
cat > "$ws/MODULE.bazel" <<EOF
module(name = "protoccmakescriptnested", version = "0.0.0")
...
echo "ok  meta-cmake-protoc-cmake-script-wrapper-nested: //:use_foo builds from the recursively-recovered cc_proto_library"
