#!/bin/sh
# meta-cmake-cmake-script-tempdir-relocate-filecopy.sh — render+build gate for the
# temp-dir-then-copy codegen recovery (file(COPY) form).
#
...
# final path in argv) and then `file(COPY) <tmp>/value.c DESTINATION <dir>` (a cmake command, not an execute_process).
...
fixture="$repo_root/converter/testdata/sample-projects/cmake-script-tempdir-relocate-filecopy"
work_dir="$(mktemp -d)"
...
echo "ok  meta-cmake-cmake-script-tempdir-relocate-filecopy: temp-dir tool recovered to a regenerating genrule + lifted copy (no frozen copy-bake)"
...
if command -v bazel >/dev/null; then
    ...
    echo "ok  meta-cmake-cmake-script-tempdir-relocate-filecopy: bazel not on PATH, skipping build half"
    exit 0
fi
...
if [ "$bazel_major" -lt 7 ]; then
    echo "ok  meta-cmake-cmake-script-tempdir-relocate-filecopy: bazel < 7, skipping build half"
    exit 0
fi
...
module(name = "tdrelocatefc", version = "0.0.0")
...
echo "ok  meta-cmake-cmake-script-tempdir-relocate-filecopy: //:app builds + runs from the recovered temp-dir tool genrule"
