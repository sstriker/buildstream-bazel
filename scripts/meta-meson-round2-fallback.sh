...
for marker in \
    ...
    if ! grep -qF -- "$marker" "$mod_a"; then
        echo "meta-meson-round2-fallback: A MODULE.bazel missing marker: $marker" >&2
        cat "$mod_a" >&2
        exit 1
    fi
done
