...
- **`execute_process` codegen → recognizer / genrule re-run parity (over the derived-output bake).** The codegen-recognizer registry (`recognizeCodegenWith`) is wired only into the `add_custom_command` path (`standalone_genrules.go:458`); `execute_process` never reaches it, so a `protoc` run via `execute_process` falls to the derived-name stem-match **bake** (`bakeDerivedOutputs`) while the same tool via `add_custom_command` lifts to `proto_library` + `cc_proto_library`. Two complementary closures, both keying off the codemodel's record of the output (`cc.ConsumedBuildRel`): (1) route the unspecified-output `execute_process` through `recognizeCodegenWith` before the bake — sound via the recognizer's output cross-check, gated on the existing `--recognize-codegen`; (2) a placement-aware generic **genrule re-run** over the bake for the stem-match class (declare `outs` from the codemodel, `cd $(RULEDIR)` for CWD-relative writers; input-relative writers stay baked since Bazel srcs are read-only). The recognizer wrinkle: its consumer wiring (`NativeRuleConsumerLabels`, `emit/bazel/split.go`) is `#include`-header-oriented, but the typical `execute_process`+protoc shape has the generated `.pb.cc` as a codemodel **source** of a target — lifting it needs that source dropped from the consumer's `srcs`/`hdrs` and replaced by a dep on the native rule. See `docs/design/execute-process-recovery.md` for the full state machine. Demand signal: a corpus member whose codegen runs via `execute_process` rather than `add_custom_command`.
