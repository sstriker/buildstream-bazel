...
- **Complexity lens — soft-launched, drive to green then flip to blocking.**
  ...
  gocognit>50, nestif>10, funlen); the burndown is underway (**47** now — passes
  1-7 have cleared seven mid-tier functions (`lowerInterfaceLibraries`,
  `applyProbeGenexProperties`, `lowerDirectoryInstallers`,
  `stageGeneratedSourceRootIncludes`, `recoverSourceComments`,
  `buildFindPackageAttrib`, `fileapi.Load`) via behavior-preserving extraction of
  cohesive sub-passes into helpers — the established pattern). The
