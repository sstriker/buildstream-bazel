...
// pthread, ${CMAKE_DL_LIBS} → dl, a bare -lm) as cc_library linkopts. Without
...
// -l prefix; an already-flag fragment passes through.
func TestGenerate_LinkLibrariesToLinkopts(t *testing.T) {
	im := &manifest.Imports{
		...
		Elements: []*manifest.Element{{
			...
			Exports: []*manifest.Export{{
				...
				LinkLibraries: []string{"m", "pthread", "-Wl,--as-needed"},
			}},
		}},
	}
	...
	s := string(build)
	for _, want := range []string{`    linkopts = [`, `"-lm"`, `"-lpthread"`, `"-Wl,--as-needed"`} {
		if !strings.Contains(s, want) {
			t.Errorf("wrapper BUILD missing linkopts %q:\n%s", want, s)
		}
	}
}
