# rules_buildstream_bazel sources extension.
#
# Declares one external repo per source identity in the .bst
# graph (read from tools/sources.json by the rendered project's
# MODULE.bazel via the from_json tag). Each repo's ":tree"
# filegroup ctx.symlinks into the FUSE mount (cas-fuse /
# bb_clientd) so source bytes live in CAS but resolve as on-disk
# paths from Bazel's perspective.
#
# Configuration:
...
''')
    ...
    rctx.file("WORKSPACE", "")
    rctx.file("BUILD.bazel", '''# Generated by rules_buildstream_bazel//rules:sources.bzl.
#
# Two access shapes:
