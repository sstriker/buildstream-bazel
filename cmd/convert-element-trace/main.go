...
func main() {
	...
	outIRJSON := flag.String("out-ir-json", "", "optional: path to write the recovered ir.Package as JSON, parallel to convert-element-cmake's --out-ir-json. Drives the orchestrator's per-element multi-platform fold for round-2 trace-driven kinds; ignored by single-platform flows. The legacy BUILD.bazel.out at --out-build is emitted independently.")
	...
	bazelPackagePath := flag.String("bazel-package-path", "", "repo-root-relative path of the destination Bazel package (e.g. \"elements/foo\"). Frames the emitted `# gazelle:cc_search` directives so gazelle_cc's resolver — which interprets cc_search arguments repo-root relative — picks up the same include search paths the trace recorded. Empty suppresses the directive.")
	flag.Parse()
	...
	out, err := renderRules(rules, *bazelPackagePath)
	if err != nil {
		// Reaching this branch means converter/emit/bazel's
		// template execution failed — a programming error in
		// the shared emitter, not anything the operator can
		// fix. Exit non-zero so the converter genrule fails
		// loudly rather than silently writing a malformed
		// BUILD.bazel.out that a later bazel-build would
		// reject with a less actionable diagnostic.
		fmt.Fprintf(os.Stderr, "convert-element-trace: render: %v\n", err)
		os.Exit(1)
	}
...
func writePlaceholderBuild(outBuild string) error {
	...
	// Reuse traceBuildHeader (defined alongside renderRules) so
	// every BUILD.bazel.out convert-element-trace writes — render
	// path, empty-rules short-circuit, and this round-2 boot
	// placeholder — carries an identical first line. A reader
	// can grep `# Generated by convert-element-trace. DO NOT
	// EDIT.` to find every output the trace converter produced.
	body := traceBuildHeader + `# Round-2 boot phase: no trace published for this srckey yet.
...
// traceBuildHeader is the file-head provenance comment for
// every BUILD.bazel.out convert-element-trace produces — both
// via the normal render path (renderRules → bazel.EmitWithOptions
// with Options.Header set) and via writePlaceholderBuild's
// round-2-boot empty placeholder. Keeping a single constant
// guarantees both paths attribute identically; gazelle / a
// human reader can grep this string to find every output
// convert-element-trace produced.
const traceBuildHeader = "# Generated by convert-element-trace. DO NOT EDIT.\n\n"

// emitBuild renders the recovered ruleset as a BUILD.bazel
// string by routing through converter/emit/bazel — the same
// renderer the cmake / meson / fold paths use. Folding to the
// shared renderer (rather than the previous inline format-
...
// repo. Documented in ROADMAP.md
// Phase 1.
//
// Wraps renderRules with a `string`-returning signature for
// test callers that don't want to thread error handling
// through trivial assertions. Panics on a bazel.Emit error —
// such errors are unreachable in practice (the shared
// emitter's templates are statically valid) so a panic here
// is the right shape for a programming-error path. Production
// callers (main()) use renderRules directly and propagate the
// error via os.Exit so an invalid BUILD never silently lands
// on disk.
func emitBuild(g *Graph, imports *manifest.Resolver, makeDB *MakeDB, generatedHeaders []string) string {
	...
	out, err := renderRules(rules, "")
	if err != nil {
		panic(fmt.Sprintf("convert-element-trace: emitBuild: unreachable bazel.Emit error: %v", err))
	}
	return out
}

// renderRules serialises a pre-computed []CCRule slice to
// BUILD.bazel.out text via converter/emit/bazel.EmitWithOptions
...
// computation, and so main() can exit non-zero on the
// (unreachable-in-practice) bazel.Emit error path instead of
// silently producing an invalid BUILD.
//
// Empty-rules case: returns a short standalone marker comment
// (legacy "no buildable targets recovered from trace" shape
// preserved from the pre-fold renderer). bazel.Emit on an
// empty package produces a valid-but-uninformative BUILD —
// header + `package(default_visibility=...)` — that doesn't
// communicate to the operator that the trace had no
// recoverable rules. Short-circuiting here preserves the
// diagnostic and keeps the round-2 boot phase's "no targets
// yet" signal distinguishable from "targets emitted".
func renderRules(rules []CCRule, bazelPackagePath string) (string, error) {
	if len(rules) == 0 {
		return traceBuildHeader + "# (no buildable targets recovered from trace)\n", nil
	}
	pkg := toIR(rules)
	body, err := bazel.EmitWithOptions(&pkg, bazel.Options{
		Header:           traceBuildHeader,
