...
func emitBuild(g *Graph, imports *manifest.Resolver, makeDB *MakeDB, generatedHeaders []string) string {
	return renderRules(recoveredRules(g, imports, makeDB, generatedHeaders))
}

// renderRules is the rendering half of emitBuild: it serialises
// a pre-computed []CCRule slice to BUILD.bazel.out text. Factored
// out so the --out-ir-json path (which also needs the same
// post-fold ruleset) can share the buildRules + generated-headers
// computation with the BUILD.bazel render rather than recomputing
// in toIR.
