...
# kind:cmake round-2 fallback — Project B install (TreeArtifact).
# Wraps cmake configure + ninja + install under build-tracer,
# installs into the pipeline_install install-root TreeArtifact,
# and publishes the trace to the REAPI ActionCache (when
# CAS_GRPC_ADDR is set).
#
...
%[6]s    command = """
        ...
        for src in @@SRCS@@; do
            ...
            #     generated source flows through this action).
        ...
        # The install-root TreeArtifact directory. cmake installs
        # DIRECTLY into it (no tar): the entries inside are
        # <dest>/<artefact> (matching cmake's install destinations
        # + NameOnDisk), which the placeholder shape's pick_file
...
%[2]s
        ...
        cp -L "$$CMAKE_TRACE" "$$EXEC_ROOT/@@OUT:%[4]strace.log@@"
...
%[7]s
        ...
        # → re-runs this install action. trace-publish itself
        # ALSO short-circuits on empty --cas; checking here
        # keeps the action output readable when debugging.
        if [ -n "$${CAS_GRPC_ADDR:-}" ]; then
            ...
            # of N per-platform actions publishes under its own
