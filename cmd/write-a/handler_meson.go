...
func (mesonHandler) RenderA(elem *element, elemPkg string) error {
	...
	// Round-2 fallback: pre-compute the srckey hex for the
	// trace_load attr. Mirrors handler_cmake's RenderA shape; the
	// publish side (project B) renders srckey.txt for trace-publish
	// to read at action time, but project A's trace_load rule takes
	// the hex as a string attr rather than reading the file —
	// simpler input set, no need to stage srckey.txt into A.
	srckeyHash := ""
	if mesonConfig.round2FallbackEnabled {
		hash, _, err := computeSrckey(elem, mesonSrckeyPatterns())
		if err != nil {
			return fmt.Errorf("element %q: compute srckey for trace_load: %w", elem.Name, err)
		}
		srckeyHash = hash
	}
	return writeFile(filepath.Join(elemPkg, "BUILD.bazel"), mesonElementBuildA(elem, hasImports, srckeyHash))
}
...
func mesonElementBuildA(elem *element, hasImports bool, srckeyHash string) string {
...
`)
	...
	// to trace-lookup at action time. Mirrors kind:cmake's round-2
	// wiring in handler_cmake.go. expect_make_db=False because
...
`, elem.Name)
	...
	if mesonConfig.round2FallbackEnabled {
		...
		// (see rules/traces.bzl); AC hit ⇒ trace.log materializes
		// into the rule's declared outputs + the marker reports
		// "hit"; AC miss ⇒ zero-byte trace.log + "miss" marker.
		// convert-element-meson doesn't consume the trace yet
		// (trace-driven convergence research follow-on teaches it
		// to refine refusals); wiring the lookup now means that
		// follow-on is converter-side only. Mirrors kind:cmake's
		// round-2 wiring.
